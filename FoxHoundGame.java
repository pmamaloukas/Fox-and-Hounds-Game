...
public class FoxHoundGame {
    ...
    public static String[] updatePositions(int dim, String[] players, String[] move){ //helper function to update the players
        //array everytime a new valid move is made.
    ...
    private static void gameLoop(int dim, String[] players) {
        ...
        while(!exit) {
            ...
            if (choice == 1){
                // handle menu choice
                String[] move = FoxHoundUI.positionQuery(dim, STDIN_SCAN);
                String origin = move[0];
                String dest = move[1];
                if(FoxHoundUtils.isValidMove(dim,players,turn,origin,dest)){
                    switch(choice) {
                    ...
                    players = updatePositions(dim, players, move);
